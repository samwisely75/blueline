...
/// Enhanced Command trait that returns more detailed results.
///
/// This is an evolution of the basic Command trait that provides
/// better information for rendering decisions.
pub trait CommandV2 {
    ...
    /// efficient. Commands should check mode, pane, key codes, and modifiers
    /// here to determine if they should handle the event.
    ///
    /// # Arguments
    ///
    ...
    /// # Returns
    ///
    /// * `true` - This command should process the event
    /// * `false` - This command should ignore the event
    ///
    /// The default implementation returns true (always relevant).
    fn is_relevant(&self, event: KeyEvent, state: &AppState) -> bool {
        let _ = (event, state); // Suppress unused parameter warnings
        true
    }

    /// Process a key event with detailed result information.
    ///
    /// This method should only be called if `is_relevant()` returns true.
    /// It can assume the event is relevant and focus on the actual business logic.
...
// Blanket implementation to allow gradual migration
