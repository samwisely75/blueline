...
/// Type alias for cursor movement result to reduce complexity
type CursorMoveResult = (LogicalPosition, Option<ModelEvent>);
    ...
    pub fn insert_text(&mut self, pane: Pane, position: LogicalPosition, text: &str) -> ModelEvent {
        ...
        if text.contains('\n') {
            ...
            if lines.len() > 1 {
                ...
                self.lines
                    .insert(position.line + lines.len() - 1, last_line);
            }
        } else {
...
impl BufferModel {
    ...
    pub fn move_cursor_left(&self, current_pos: LogicalPosition) -> CursorMoveResult {
    ...
    /// Returns new cursor position and optional event
    pub fn move_cursor_right(&self, current_pos: LogicalPosition) -> CursorMoveResult {
        let old_pos = current_pos;
...
mod tests {
    ...
    fn buffer_content_should_insert_single_line_text() {
        ...
        assert_eq!(
            content.get_line(1).unwrap(),
            "line2 inserted with more text"
        );
        match event {
            ModelEvent::TextInserted {
                pane,
    ...
    fn buffer_model_should_move_cursor_left_within_line() {
        ...
        let _ = model.content_mut().insert_text(
            Pane::Request,
    ...
    fn buffer_model_should_move_cursor_right_to_next_line() {
        let model = BufferModel::with_content(Pane::Request, create_test_content());
        let current_pos = LogicalPosition {
            line: 1,
            column: 20,
        }; // End of "line2 with more text"
    ...
    fn buffer_content_should_validate_positions() {
        ...
        assert!(content.is_valid_position(LogicalPosition {
            line: 1,
            column: 20
        }));
    ...
    fn buffer_content_should_clamp_positions() {
        ...
        let clamped = content.clamp_position(LogicalPosition {
            line: 10,
            column: 50,
        });
