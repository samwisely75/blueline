...
impl Command for MoveRightCommand {
    fn is_relevant(
        ...
        match key_event.code {
            // 'l' key in navigation modes without modifiers
            KeyCode::Char('l') => key_event.modifiers.is_empty(),
            // Right arrow key without Shift or Control (allows Alt for other purposes)
            KeyCode::Right => {
...
mod tests {
    ...
    #[test]
    fn move_right_should_not_be_relevant_in_non_movement_modes() {
        let command = MoveRightCommand::new();
        let key_event = create_key_event(KeyCode::Char('l'), KeyModifiers::NONE);
        let context = CommandContext {
