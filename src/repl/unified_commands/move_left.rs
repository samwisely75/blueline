...
impl Command for MoveLeftCommand {
    ...
    fn execute(&self, context: &mut ExecutionContext) -> Result<Vec<ViewEvent>> {
        ...
        match context.app_state.move_cursor_left() {
            Ok(()) => {
                tracing::debug!("Cursor moved left successfully");
                // Return events for UI updates - the move_cursor_left() method
                // internally handles view event emission, so we return empty here
                // to avoid duplicate events
                Ok(vec![])
            }
            Err(e) => {
                tracing::warn!("Failed to move cursor left: {}", e);
                context
                    .app_state
                    .set_status_message("Cannot move cursor left".to_string());
