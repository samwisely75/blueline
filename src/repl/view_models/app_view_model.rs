...
impl AppViewModel {
    ...
    pub fn set_mode(&mut self, mode: EditorMode) {
        ...
        if self.state.event_bus.is_some() {
            self.state.pending_view_events.push(ViewEvent::StatusBarUpdateRequired);
        }
    }
    ...
    pub fn switch_pane(&mut self, pane: Pane) {
        if self.state.active_pane != pane {
            ...
            if self.state.event_bus.is_some() {
                self.state.pending_view_events.push(ViewEvent::FocusSwitched);
                self.state.pending_view_events.push(ViewEvent::StatusBarUpdateRequired);
            }
        }
    }
    ...
    pub fn update_dimensions(&mut self, width: u16, height: u16) {
        ...
        self.state.request_pane.update_dimensions(width as usize, height as usize);
        self.state.response_pane.update_dimensions(width as usize, height as usize);
        
        // Emit full redraw event for resize
        if self.state.event_bus.is_some() {
            self.state.pending_view_events.push(ViewEvent::FullRedrawRequired);
        }
    }
    ...
    pub fn set_cursor_position(&mut self, position: LogicalPosition) {
        self.state.active_pane_state_mut().buffer.set_cursor(position);
        
        // Emit cursor update event
        if self.state.event_bus.is_some() {
            self.state.pending_view_events.push(ViewEvent::ActiveCursorUpdateRequired);
            self.state.pending_view_events.push(ViewEvent::PositionIndicatorUpdateRequired);
        }
    }
